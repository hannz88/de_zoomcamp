...
def create_events_aggregated_sink(t_env):
    table_name = 'processed_events_aggregated2'
    sink_ddl = f"""
...
--             lpep_dropoff_datetime VARCHAR,
            ...
            trips_count BIGINT,
            PRIMARY KEY (window_start, PULocationID, DOLocationID, trips_count) NOT ENFORCED
        ) WITH (
            'connector' = 'jdbc',
...
def create_events_source_kafka(t_env):
    table_name = "events"
    pattern = "yyyy-MM-dd HH:mm:ss" #created this myself
    source_ddl = f"""
        CREATE TABLE {table_name} (
            lpep_pickup_datetime VARCHAR,
            lpep_dropoff_datetime VARCHAR,
    ...
    try:
        ...
        INSERT INTO {aggregated_table}
            (SELECT DISTINCT
             ...
             FROM TABLE(TUMBLE(TABLE {source_table}, DESCRIPTOR(event_watermark), INTERVAL '5' MINUTE))
            --from TABLE(SESSION (TABLE {source_table}, DESCRIPTOR(event_watermark), INTERVAL '5' MINUTE))
